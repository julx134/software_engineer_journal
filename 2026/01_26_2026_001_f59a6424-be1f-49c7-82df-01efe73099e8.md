...
**PyArmor** is a command-line tool used to obfuscate and license Python scripts. It works by encrypting Python bytecode and wrapping it with a runtime protection module. When the obfuscated script runs, the PyArmor runtime (a dynamic library) decrypts the bytecode just-in-time for execution, preventing static analysis of the source code.

There are two primary modes of operation relevant to this project:

1.  **Standard Obfuscation (`pyarmor gen`)**: This is the default mode. It encrypts the code object (co_code) within the compiled Python files but preserves the structure of the application, including function names, class names, and variable names.
2.  **Rename Function/Type (`--enable-rft`)**: This is an advanced mode that performs source-to-source transformation _before_ compilation. It aggressively renames functions, arguments, and classes to meaningless strings (e.g., changing `verify_license` to `f_x1`).

**The Risk of Standard Mode:**
While `pyarmor gen` prevents casual snooping, it relies heavily on the encryption of the bytecode. If an attacker can dump the bytecode from memory during runtime, the logic becomes exposed. Crucially, because standard mode preserves symbol names for compatibility, the dumped code is highly readable. An attacker can easily infer business logic by reading function names like `calculate_tax` or `validate_user`, even if the exact instruction flow is slightly obscured.
