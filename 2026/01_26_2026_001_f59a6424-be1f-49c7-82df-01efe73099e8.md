...
2.  **RFT Mode (`--enable-rft`)**: This stands for "Rename Function/Type." It’s the aggressive option. Before compiling, it rewrites the source code and changes readable names (like `check_password`) into random nonsense (like `f_x1`).

**The Risk of Standard Mode:**
The issue with just running standard `pyarmor gen` is that it relies almost entirely on the encryption. If a smart attacker manages to dump the memory while the app is running, they can grab the decrypted bytecode. Since standard mode keeps all the original names to play nice with frameworks, the dumped code is super easy to read. Even if they can't see the exact original source, seeing a function named `validate_license_key` pretty much tells them exactly where to look to bypass our security.
