...
1. **JDK Proxies:** This applies to any class implementing an interface. The interface methods define exactly what can be proxied.
...
> _Updated 03/01/2026:_ After working on **_*02-28-2026 Feature Request (687601d5-4948-41be-8a4d-0929606de2b2)*_**, I found that with the latest versions of JVM (java 8+) and Spring (spring 4+), a library called **Objenesis** allows Spring to instantiate CGLIB Proxy objects without having to call the constructors twice. Essentially, with most modern Java and Spring applications, there should be no concerns regarding calling constructors twice.
