...
From this point, I tried to pivot back to using Shaka's built-in metadata callback function, particularly the `addEventListener('metadata', () => {})` function. I did some digging around the documentation and online discussions and I found that Shaka also supported `emsg` metadata events. The `emsg` metadata events present another in-band approach of muxing metadata with video particularly for the CMAF protocol. That sparked the discussion I had with the senior engineer regardin the MISB ST 1910 standard, which defines the encoding of KLV metadata in an ABR protocol, specifically using the Common Media Application Format (CMAF). As part of the standard and CMAF protocol, emsg boxes were used as a wrapper around the KLV metadata.

I did some feasibility testing which included the use of MPEG-DASH as the ABR protocol to segment the video frames into .m4s/.mp4 format as per the standard. As the metadata is demuxed from the MPEG-TS stream, a callback is executed to write a custom `emsg` box and prepend it to the segments. `emsg` stands for Event Message and is where developers can declare metadata such as KLV metadata for the CMAF container. The segments are then requested by Shaka.js in the frontend in which a callback is called in the presence of emsg boxes or metadata information. First, I validated that the metadata is indeed part of the segments downloaded by the frontend, however, Shaka failed to recognize the metadata present in the segments.

My next plan was to try Dash.js because as part of my discussion with the senior engineer, he gave me documentation of the standard as well as a feasibility testing report carried out by MISB. This report entailed implementing ST 1910 using Dash.js in the frontend, so I went to implement this path. However, Dash.js also failed to recognize the metadata present.Further debugging lead to the conclusion that the CMAF container, along with the standard, is too strict with video encoding as well as segment formats/manifests. This means that I would have to change the Muxer module because it needs strictly formated h264 stream and significant research must be conducted to gain knowledge on this formatting and concepts (i.e. IDR, SPS/PPS headers, moof, moov, etc…)
